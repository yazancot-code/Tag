...
│   ├── screens/
│   │   └── HomeScreen.tsx     — Tag list + inline NFC scanning with countdown timer
│   ├── database/
│   │   └── database.ts        — SQLite init, insertTag, getAllTags
│   ├── utils/
│   │   └── nfc.ts             — NFC utility functions (extractTextPayload)
│   ├── types/
│   │   └── index.ts           — TypeScript interfaces
│   └── components/
│       └── TagListItem.tsx    — Tag row component
└── ...
```
...
| Screen | Purpose |
|---|---|
| **HomeScreen** | Tag list + inline NFC scanning with 10s countdown. Auto-scans on mount; cycles: scan (10s timer) → tag found (2s pause) → auto-rescan; timeout shows Rescan button. |
